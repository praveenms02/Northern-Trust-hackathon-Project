...
async function login() {
  ...
  try {
    ...
    if (data.success) {
      ...
      document.getElementById("sidebarBalance").textContent = "₹ " + data.balance.toLocaleString("en-IN");
      showPage("dashboard");
      loadAllUsers();
      loadHistory();
    ...
    tbody.innerHTML = data.payments.map(p => {
      ...
      if (p.type === "SENT" && p.status === "SUCCESS") {
        if      (rs === "accepted") refundBtn = `<span class="refund-done refund-accepted">✅ Refunded</span>`;
          ...
          refundBtn = `<span class="refund-expired">↩ Expired</span>`;
        }
      } else { refundBtn = "—"; }
      return `<tr>
...
async function checkStatus() {
  ...
  try {
    ...
    card.innerHTML = rows.map(([k,v]) => `<div class="status-row"><span class="key">${k}</span><span class="val">${v}</span></div>`).join("");
  } catch (e) { card.innerHTML = `<p style="color:var(--red)">Connection error.</p>`; }
}
...
async function submitRefundRequest() {
  ...
  } catch (e) { errEl.textContent = "Connection error."; errEl.classList.add("show"); }
...
async function actionRefund(refundId, action) {
  ...
  try {
    ...
    const data = await res.json();
    if (data.success) {
      showToast(action === "ACCEPT" ? "✅ Refund accepted! Amount transferred back." : "❌ Refund request rejected.",
...
async function loadAnalytics() {
  try {
    ...
    document.getElementById("statVolume").textContent  = "₹ " + s.totalVolume.toLocaleString("en-IN");
    const div = document.getElementById("failureBreakdown");
    if (!s.failureBreakdown || Object.keys(s.failureBreakdown).length === 0) {
      div.innerHTML = '<p style="color:var(--muted);font-size:0.875rem">No failure data yet.</p>';
      return;
    }
    const total = Object.values(s.failureBreakdown).reduce((a, b) => a + b, 0);
    div.innerHTML = Object.entries(s.failureBreakdown).sort((a,b) => b[1]-a[1]).map(([reason, count]) => {
